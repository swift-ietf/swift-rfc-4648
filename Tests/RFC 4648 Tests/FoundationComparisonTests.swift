...
import Testing
    ...
    struct FoundationComparisonTests {
        ...
        func `Base64 encoding matches Foundation`() {
            let testCases: [[Byte]] = [
                [],
                "f".utf8.map(Byte.init(bitPattern:)),
                "fo".utf8.map(Byte.init(bitPattern:)),
                "foo".utf8.map(Byte.init(bitPattern:)),
                "foob".utf8.map(Byte.init(bitPattern:)),
                "fooba".utf8.map(Byte.init(bitPattern:)),
                "foobar".utf8.map(Byte.init(bitPattern:)),
                ([0x00, 0xFF, 0x80, 0x7F] as [UInt8]).map(Byte.init(bitPattern:)),
                "The quick brown fox jumps over the lazy dog".utf8.map(Byte.init(bitPattern:)),
                (0..<100).map { Byte(bitPattern: UInt8($0 % 256)) },
            ]

            for bytes in testCases {
                let ourEncoding = String.base64(bytes, padding: true)
                let foundationEncoding = Data(bytes.map(\.bitPattern)).base64EncodedString()
        ...
        func `Base64 decoding matches Foundation`() {
            ...
            for encoded in testCases {
                ...
                let foundationDecoding = Data(base64Encoded: encoded).map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 round-trip matches Foundation`() {
            let testBytes: [[Byte]] = [
                "Hello, World!".utf8.map(Byte.init(bitPattern:)),
                (0..<255).map { Byte(bitPattern: UInt8($0)) },
                Array(repeating: Byte(bitPattern: 0xFF), count: 100),
                Array(repeating: Byte(bitPattern: 0x00), count: 100),
            ]
            ...
            for bytes in testBytes {
                ...
                let foundationEncoded = Data(bytes.map(\.bitPattern)).base64EncodedString()
                let foundationDecoded = Data(base64Encoded: foundationEncoded).map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 encoding with line length matches Foundation`() {
            let longBytes = (0..<200).map { Byte(bitPattern: UInt8($0 % 256)) }

            let ourEncoded = String.base64(longBytes)

            let foundationEncoded = Data(longBytes.map(\.bitPattern)).base64EncodedString()
        ...
        func `Base64 whitespace handling - RFC 4648 compliance`() {
            ...
            #expect(ourDecoded == "foobar".utf8.map(Byte.init(bitPattern:)))
            ...
            let foundationClean = Data(base64Encoded: withoutWhitespace).map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 empty string handling`() {
            ...
            let foundationEncoded = Data(emptyBytes.map(\.bitPattern)).base64EncodedString()

            #expect(ourEncoded == foundationEncoded)
            #expect(ourEncoded.isEmpty)

            let ourDecoded = [Byte](base64Encoded: "")
            let foundationDecoded = Data(base64Encoded: "").map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 large data matches Foundation`() {
            ...
            let largeBytes = (0..<(1024 * 1024)).map { Byte(bitPattern: UInt8($0 % 256)) }

            let ourEncoded = String.base64(largeBytes)
            let foundationEncoded = Data(largeBytes.map(\.bitPattern)).base64EncodedString()
        ...
        func `Base64 all byte values match Foundation`() {
            let allBytes = (0...255).map { Byte(bitPattern: UInt8($0)) }

            let ourEncoded = String.base64(allBytes)
            let foundationEncoded = Data(allBytes.map(\.bitPattern)).base64EncodedString()
        ...
        func `Hex encoding produces valid output`() {
            let testBytes: [Byte] = ([0x00, 0x0F, 0xFF, 0xAB, 0xCD, 0xEF] as [UInt8]).map(Byte.init(bitPattern:))
        ...
        func `Base64 all single-byte values match Foundation`() {
            for byte in 0...255 {
                let bytes: [Byte] = [Byte(bitPattern: UInt8(byte))]
        ...
        func `Base64 all two-byte combinations (sampled)`() {
            ...
            for i in stride(from: 0, through: 255, by: 17) {
                for j in stride(from: 0, through: 255, by: 17) {
                    let bytes: [Byte] = [Byte(bitPattern: UInt8(i)), Byte(bitPattern: UInt8(j))]
            ...
            for i in stride(from: 0, through: 255, by: 51) {
                for j in stride(from: 0, through: 255, by: 51) {
                    for k in stride(from: 0, through: 255, by: 51) {
                        let bytes: [Byte] = [Byte(bitPattern: UInt8(i)), Byte(bitPattern: UInt8(j)), Byte(bitPattern: UInt8(k))]
        ...
        func `Base64 specific lengths match Foundation`(length: Int) {
            let bytes = (0..<length).map { Byte(bitPattern: UInt8($0 % 256)) }
        ...
        func `Base64 random data patterns match Foundation`() {
            ...
            for _ in 0..<100 {
                ...
                let bytes: [Byte] = (0..<length).map { _ in
                    Byte(bitPattern: UInt8.random(in: 0...255, using: &generator))
                }
        ...
        func `Base64 padding scenarios match Foundation`(length: Int, expectedPattern: String) {
            let bytes: [Byte] = Array(repeating: Byte(bitPattern: 0), count: length)
        ...
        func `Base64 BinaryInteger UInt8 values match Foundation`() {
            for value in [UInt8.min, 1, 127, 128, 255, UInt8.max] {
                let bytes = withUnsafeBytes(of: value.bigEndian) { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 consecutive byte values match Foundation`() {
            for start in stride(from: 0, through: 200, by: 50) {
                let length = 55
                let bytes = (start..<min(start + length, 256)).map { Byte(bitPattern: UInt8($0)) }
        ...
        func `Base64 alternating patterns match Foundation`() {
            let patterns: [[Byte]] = [
                Array(repeating: ([0x00, 0xFF] as [UInt8]).map(Byte.init(bitPattern:)), count: 50).flatMap { $0 },
                Array(repeating: ([0xAA, 0x55] as [UInt8]).map(Byte.init(bitPattern:)), count: 50).flatMap { $0 },
                Array(repeating: ([0x00, 0x80, 0xFF] as [UInt8]).map(Byte.init(bitPattern:)), count: 50).flatMap { $0 },
                (0..<100).map { Byte(bitPattern: UInt8($0 % 2 == 0 ? 0xFF : 0x00)) },
            ]

            for pattern in patterns {
                let ourEncoded = String.base64(pattern)
                let foundationEncoded = Data(pattern.map(\.bitPattern)).base64EncodedString()
        ...
        func `Base64 decode various valid inputs match Foundation`() {
            ...
            for encoded in validInputs {
                ...
                let foundationDecoded = Data(base64Encoded: encoded).map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 decode with padding matches Foundation`() {
            ...
            let testCases: [(padded: String, expected: [Byte])] = [
                ("YQ==", "a".utf8.map(Byte.init(bitPattern:))),
                ("YWI=", "ab".utf8.map(Byte.init(bitPattern:))),
                ("YWJj", "abc".utf8.map(Byte.init(bitPattern:))),
                ("YWJjZA==", "abcd".utf8.map(Byte.init(bitPattern:))),
            ]
            ...
            for (padded, expectedBytes) in testCases {
                ...
                let foundationDecoded = Data(base64Encoded: padded).map { $0.map(Byte.init(bitPattern:)) }
        ...
        func `Base64 very large data matches Foundation`() {
            ...
            let largeBytes = (0..<largeSize).map { Byte(bitPattern: UInt8($0 % 256)) }
        ...
        func `Base64 repetitive patterns at scale match Foundation`() {
            let patterns: [[Byte]] = [
                Array(repeating: Byte(bitPattern: 0x00), count: 10000),
                Array(repeating: Byte(bitPattern: 0xFF), count: 10000),
                Array(repeating: Byte(bitPattern: 0xAA), count: 10000),
                Array(repeating: Byte(bitPattern: 0x55), count: 10000),
            ]
